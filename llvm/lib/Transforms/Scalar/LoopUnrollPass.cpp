  ...
  if (TripCount) {
    ...
    if (UP.PartialThreshold != NoThreshold) {
      ...
      if (UP.AllowRemainder && count <= 1) {
        ...
        while (count != 0 && UCE.getUnrolledLoopSize(UP) > UP.PartialThreshold)
          count >>= 1;
      }
      if (count < 2) {
        count = 0;
...
bool llvm::computeUnrollCount(
  ...
  PInfo.UserUnrollCount = UserUnrollCount;
  PInfo.PragmaFullUnroll = PragmaFullUnroll;
  PInfo.PragmaCount = PragmaCount;
  PInfo.PragmaEnableUnroll = PragmaEnableUnroll;
